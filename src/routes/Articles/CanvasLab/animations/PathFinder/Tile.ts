...
export class Tile {
	...
	constructor(params: TileParams) {
	  ...
	  // wall colors
	  this.defaultWallColor =	params.defaultWallColor ?? new Uint8ClampedArray(LIGHT_GRAY);
	  this.currentWallColor =	params.currentWallColor ?? new Uint8ClampedArray(LIGHT_GRAY);

	  // fill colors
	  this.defaultOpenFillColor =	params.defaultOpenFillColor ?? new Uint8ClampedArray(WHITE);
	  this.defaultWallFillColor =	params.defaultWallFillColor ?? new Uint8ClampedArray(BLACK);
	  this.solveFillColor =	params.solveFillColor ?? new Uint8ClampedArray(ELETRIC_BLUE);
	  this.initialSearchFillColor = params.initialSearchFillColor
			?? new Uint8ClampedArray(ELETRIC_BLUE);
	  this.finalSearchFillColor =	params.finalSearchFillColor ?? new Uint8ClampedArray(LIGHT_YELLOW);
	  this.currentFillColor =	params.currentFillColor ?? new Uint8ClampedArray(WHITE);
	  this.generationFillColor =	params.generationFillColor ?? new Uint8ClampedArray(ELETRIC_BLUE);

	  this.tileWidth =	(this.grid.canvas.width - this.grid.padding) / this.grid.array.length;
...
&& this.grid.array[this.col + colOffset][this.row + rowOffset])
  ...
  */
	getTraversableNeighbors(): Tile[] {
	  ...
	  if (northTile && northTile.type !== 'wall') traversableNeighbors.push(northTile);
	  const eastTile = this.getEastTile();
	  if (eastTile && eastTile.type !== 'wall') traversableNeighbors.push(eastTile);
	  const southTile = this.getSouthTile();
	  if (southTile && southTile.type !== 'wall') traversableNeighbors.push(southTile);
	  const westTile = this.getWestTile();
	  if (westTile && westTile.type !== 'wall') traversableNeighbors.push(westTile);
	  ...
	  for (let i = 0; i < currentColor.length; i++) {
	    if (currentColor[i] !== destinationColor[i]) {
	      ...
	      if (Math.abs(currentColor[i] - destinationColor[i]) < incrementAmount) {
	        ...
	        const direction = destinationColor[i] > currentColor[i]
	          ? incrementAmount
	          : -incrementAmount;
	        // eslint-disable-next-line no-param-reassign
