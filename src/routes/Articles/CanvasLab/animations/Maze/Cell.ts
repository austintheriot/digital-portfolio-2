...
export class Cell {
	...
	constructor(params: CellParams) {
	  ...
	  // wall colors
	  this.defaultWallColor =	params.defaultWallColor ?? new Uint8ClampedArray(BLACK);
	  this.generationWallColor = params.generationWallColor ?? new Uint8ClampedArray(ELETRIC_BLUE);
	  this.currentWallColor =	params.currentWallColor ?? new Uint8ClampedArray(BLACK);

	  // fill colors
	  this.defaultFillColor =	params.defaultFillColor ?? new Uint8ClampedArray(WHITE);
	  ...
	  this.cellWidth =			(this.maze.canvas.width - this.maze.padding) / this.maze.array.length;
